...
(defun c-comment-line-break-function-old (&optional soft)
  ;; we currently don't do anything with soft line breaks
  (let ((literal (c-in-literal))
	at-comment-col)
    (cond
...
(defun c-guess-fill-prefix (lit-limits lit-type)
  ...
  ;; it ends.
  (let (prefix-line comment-prefix)
    (if (eq lit-type 'c++)
	(save-excursion
	  ...
	  (if (> (point) (car lit-limits))
	      ...
	      ;; comment has more than one line, and it can therefore
	      ;; be used to find the comment fill prefix.
	      (setq prefix-line (point))
	    (goto-char (car lit-limits))
	    (if (and (= (forward-line) 0)
		     (< (point) (cdr lit-limits)))
		;; The line after the comment starter is inside the
		;; comment, so we can use it.
		(setq prefix-line (point))
	      ;; The comment is only one line.  Take the comment
	      ;; prefix from that line, but we'll have to determine
	      ;; the proper indentation column below.
	      (goto-char (car lit-limits))
	      (if (looking-at c-comment-prefix)
		  (setq comment-prefix
	...
	(if (and (> (point) (car lit-limits))
	  ...
	  (if (and (= (forward-line) 0)
		   ...
		   (not (and (looking-at "[ \t]*\\*/")
			     (eq (cdr lit-limits) (match-end 0)))))
	      ;; The line after the comment starter is inside the
	      ;; comment and contains more than just the ender, so we
	      ;; can use it.
	      (setq prefix-line (point))
	    ;; The comment is either one line or the next line
	    ;; contains just the comment ender.  Thus we have no
	    ;; information about a suitable comment prefix, so use
	    ;; c-comment-continuation-stars, but we'll have to
	    ;; determine the proper indentation column below.
	    (setq comment-prefix c-comment-continuation-stars)))))
    (if prefix-line
	(save-excursion
	  (goto-char prefix-line)
	  (if (looking-at (concat "[ \t]*\\(" c-comment-prefix "\\)"))
	      (cons (buffer-substring-no-properties prefix-line (match-end 0))
		...
		(cons (concat (buffer-substring-no-properties
	      ...
	      ;; prefix at all and we should just fill it as normal
	      ;; text.
	      (cons "" 0))))
      ;; The comment doesn't give any information about the
      ;; indentation column.  We'll have to temporarily insert a new
      ;; comment line and indent it to find the correct column.
      (let (end
	    buffer-read-only
	    (buffer-modified (buffer-modified-p)))
	(unwind-protect
	    (save-excursion
	      (goto-char (+ (car lit-limits) 2))
	      ;; We insert an extra non-whitespace character in case
	      ;; comment-prefix is "" or ends with whitespace.
	      (insert ?\n comment-prefix ?a)
	      (setq end (point-marker))
	      (c-indent-line)
	      (goto-char (1- end))
	      (cons (buffer-substring-no-properties (c-point 'bol)
						    (point))
		    (current-column)))
	  (when end
	    (delete-region (+ (car lit-limits) 2) end)
	    (set-marker end nil))
...
(defun c-fill-paragraph (&optional arg)
  (interactive "*P")
  (let* ((lit-limits (c-collect-line-comments (c-literal-limits nil t)))
    ...
    (when (and (>= (point) beg) (<= (point) end))
	  ...
	  (progn
		...
		;; comes first.
		(set-marker end (min end (1+ (cdr lit-limits))))
		(when (<= beg (car lit-limits))
		  ...
		  (if (eq (point) (car lit-limits))
		    ...
		    ;; comment.  We must fake a line that don't.
		    (setq tmp-pre t)))
		))
		...
		(when (>= end (cdr lit-limits))
		  ...
		  (if (and (not c-hanging-comment-ender-p)
		    ...
		    ;; cruft between it and the last word with a
		    ;; nonbreakable space and include it in the fill.
		    ;; We'll change the nonbreakable space to a normal
		    ;; space afterwards.
		    ...
		    (when (/= (point) (- (cdr lit-limits) 2))
		      (delete-region (point) (- (cdr lit-limits) 2))
		      (insert ?\240)
		      (setq nbsp-inserted t))
		    (forward-char 2)
	    ...
	    (when tmp-pre
	      ...
	      ;; comment line in the narrowed region.  Pad out with
	      ;; spaces to keep the starting column of the text.
	      (let ((fill (if fill-prefix
			      (cons fill-prefix
				    ;; Ugly way of getting the column
				    ;; after the fill prefix; it'd be
				    ;; nice with a current-column that
				    ;; works on strings..
				    (let ((start (point)))
				      (insert ?\n fill-prefix)
				      (unwind-protect (current-column)
		...
		(save-excursion
		  ...
		  (if (looking-at comment-start-skip)
			...
			(skip-chars-backward " \t"))
		    (forward-char 2))
		  ;; Move forward into the comment in case the comment
		  ;; starts in an earlier column than the end of the
		  ;; fill prefix.
		  (while (< (current-column) (cdr fill)) (forward-char))
...
(defun c-fill-paragraph-old (&optional arg)
