...
<ul>
  ...
  mode is gone.  CC Mode used to automatically set the style to
  <code>"java"</code> when Java mode is entered.  This has now been
  removed since it causes too much confusion.
  ...
  <code>"gnu"</code> for all other modes (as before).  So you won't
  notice the change if you haven't touched that variable.

  <p><li>New cleanups, <code>space-before-funcall</code> and
  <code>compact-empty-funcall</code>.  Two new cleanups have been
  added to <code>c-cleanup-list</code>:
  ...
  Keywords like <code>else</code>, <code>while</code>,
...
for (i = 0; i < 17; i++)
  ...
  in multiline strings.  Previously these two keys only moved by
  sentence in comments, which meant that sentence movement didn't work
  in strings containing documentation or other natural language text.
  ...
  <p><li>Support for autodoc comments in Pike mode.  Autodoc comments
  for Pike are used to extract documentation from the source, like
  Javadoc in Java.  Pike mode now recognize this markup in comment
  prefixes and paragraph starts.

  <p><li>The comment prefix regexps on <code>c-comment-prefix</code>
  may be mode specific.  When <code>c-comment-prefix</code> is an
  association list, it specifies the comment line prefix on a per-mode
  basis, like <code>c-default-style</code> does.  This change came
  about to support the special autodoc comment prefix in Pike mode
  only.

  <p><li>Better handling of syntactic errors.  The recovery after
  unbalanced parens earlier in the buffer has been improved; CC Mode
  now reports them by dinging and giving a message stating the
  offending line, but still recovers and indent the following lines in
  a sane way (most of the time).  An <code>else</code> with no
  matching <code>if</code> is handled similarly.  If an error is
  discovered while indenting a region, the whole region is still
  indented and the error is reported afterwards.

  <p><li>Lineup functions may now return absolute columns.  A lineup
  function can give an absolute column to indent the line to by
  returning a vector with the desired column as the first element.

  <p><li>More robust and warning-free byte compilation.  Although this
  is strictly not a user visible change (well, depending on the view
  of a user), it's still worth mentioning that CC Mode now can be
  compiled in the standard ways without causing trouble.  Some code
  have also been moved between the subpackages to enhance the
