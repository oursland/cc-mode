...
(defsubst c-forward-comment (count)
  ...
  (let ((here (point)))
    (unwind-protect
	(progn
	  ;; Some emacsen considers incorrectly that any line comment
	  ;; ending with a backslash continues to the next line, so we
	  ;; must temporarily remove the escape syntax.
	  (modify-syntax-entry ?\\ ".")
	  (if (>= count 0)
